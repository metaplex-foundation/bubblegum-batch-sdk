...
use mpl_bubblegum::accounts::{MerkleTree, TreeConfig};
...
use crate::merkle_tree_wrapper::{calc_merkle_tree_size, restore_canopy_depth_from_buffer};
...
/// This is the "top level" type for creating rollups.
/// It is used to create and write a rollup into Solana.
...
impl RollupClient {
    ...
    pub async fn prepare_tree(
        ...
        let merkle_tree_size = calc_merkle_tree_size(max_depth, max_buf_size, canopy_depth)
        ...
        let tree_config_size = size_of::<TreeConfig>();
        ...
        let tx = Transaction::new_signed_with_payer(
            &[
                ...
                system_instruction::create_account(
                    // acquire space for tree config account
                    &tree_creator.pubkey(),
                    tree_data_account,
                    self.client
                        .get_minimum_balance_for_rent_exemption(tree_config_size)
                        .await?,
                    tree_config_size as u64,
                    &spl_token::id(),
                ),
    ...
    pub async fn create_rollup_builder(&self, tree_account: Pubkey) -> std::result::Result<RollupBuilder, RollupError> {
        ...
        RollupBuilder::new(tree_account, max_depth, max_buffer_size, canopy_depth)
    }
    ...
    pub async fn finalize_tree(
        ...
        assert_eq!(tree_creator.pubkey(), rollup.tree_id);

        // tree_config_account
        let tree_config_account = pubkey_util::derive_tree_config_account(&rollup_builder.tree_account);

        // TODO: doublecheck
        let rightmost_index = if rollup.rolled_mints.is_empty() {
            0
        } else {
            rollup.rolled_mints.len() - 1
        };
        ...
        let finalize_instruction = FinalizeTreeWithRootBuilder::new()
            ...
            .rightmost_index(rightmost_index as u32)
